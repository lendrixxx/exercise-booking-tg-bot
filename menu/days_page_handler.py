...
def days_callback_query_handler(query: telebot.types.CallbackQuery) -> None:
  ...
  if selected_day == 'None':
    ...
    global_variables.USER_MANAGER.update_button_data_days_buttons_map(query.from_user.id, query.message.chat.id, button_data.days_buttons_map)
    global_variables.USER_MANAGER.update_query_data_days(query.from_user.id, query.message.chat.id, [])
  elif selected_day == 'All':
    for day in button_data.days_buttons_map:
      button_data.days_buttons_map[day] = telebot.types.InlineKeyboardButton(day + ' ✅', callback_data=button_data.days_buttons_map[day].callback_data)
    global_variables.USER_MANAGER.update_button_data_days_buttons_map(query.from_user.id, query.message.chat.id, button_data.days_buttons_map)
    global_variables.USER_MANAGER.update_query_data_days(query.from_user.id, query.message.chat.id, SORTED_DAYS)
  else:
    ...
    if selected_day in query_data.days:
      ...
      global_variables.USER_MANAGER.update_query_data_days(query.from_user.id, query.message.chat.id, query_data.days)
    else:
      button_data.days_buttons_map[selected_day] = telebot.types.InlineKeyboardButton(selected_day + ' ✅', callback_data=button_data.days_buttons_map[selected_day].callback_data)
      global_variables.USER_MANAGER.update_button_data_days_buttons_map(query.from_user.id, query.message.chat.id, button_data.days_buttons_map)
...
def days_selection_handler(user_id: int, message: telebot.types.Message) -> None:
  ...
  sent_msg = global_variables.BOT.send_message(message.chat.id, text, reply_markup=get_days_keyboard(user_id, message.chat.id), parse_mode='Markdown')
  global_variables.USER_MANAGER.update_button_data_days_selection_message(user_id, message.chat.id, sent_msg)
